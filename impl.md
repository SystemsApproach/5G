...
Introduction is an acronym (**C**entral **O**ffice **R**e-architected
as a **D**atacenter). More concretely, CORD is a blueprint for
...
which can be found elsewhere: https://guide.opencord.org.
...
[Figure 6.1](#cord) gives a schematic overview of a CORD POD. It
...
With respect to software, [Figure 6.1](#cord) shows a combination of
...
microservices. (Kubernetes is implicit, and not shown in the figure.)

To expand on this idea, [Figure 6.2](#ci-cd) gives an alternative view
of a CORD POD, abstracting away the details of *what* services it
hosts, and focusing instead on *how* those services are instantiated
...
When abstracted in this way, we can view a POD as including three
major subsystems:
...
* **Profile:** The deployment-specific collection of microservices
...
* **CI/CD Toolchain:** Used to assemble, deploy, operate, and upgrade
...
[Figures 6.1](#cord) and [6.2](#ci-cd). Each is a substantial open
...
* **Stratum:** A thin operating system that runs locally on each
  ...
  includes using *P4* to define the forwarding behavior of the
  switch’s forwarding pipeline (think of this program as a contract
  between the control and data planes), and *P4Runtime* to control
  that forwarding contract at runtime.
...
* **Trellis:** An ONOS-hosted SDN control application that implements
...
Trellis, as an SDN control application running on top of ONOS,
...
stages of spines, as shown in [Figure 6.3](#trellis). This means
