...
[Figure 4.1](#pipeline) shows the packet processing stages implemented
...
microservice (if that is helpful).
...
The last two stages in [Figure 4.1](#pipeline) (D/A conversion and the
RF front-end) are beyond the scope of this book.

While it is simplest to view the stages in [Figure 4.1](#pipeline) as
...
[Figure 4.2](#split-ran) is the one being actively pursued by the
...
[Figure 4.3](#ran-hierarchy), where a single *Central Unit (CU)*
running in the cloud serves multiple *Distributed Units (DUs)*, each
of which in turn serves multiple *Radio Units (RUs)*. Critically, the
...
in 5G is likely to make this later configuration all the more common.
...
are officially labelled as follows:
...
shown in [Figure 4.4](#rrc-split), where the RRC from
[Figure 4.1](#pipeline) is partitioned into two sub-components: the
...
Although not shown in [Figure 4.4](#rrc-split), keep in mind (from
[Figure 4.2](#split-ran)) that all constituent parts of the RRC, plus
the PDCP, form the CU.

Completing the picture, [Figure 4.5](#ran-controller) shows the
...
The example Control Apps in [Figure 4.5](#ran-controller) include a
...
The next three (RF Configuration, Semi-Persistent Scheduling, Cipher
...
an avenue for innovation in the future.
